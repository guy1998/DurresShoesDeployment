...
const url = 'http://localhost:8003/employers/'
...
const validateEmployee = (notification, employeeInfo)=>{
    if(!employeeInfo.name || !employeeInfo.surname || !employeeInfo.costPerDay){
        notification.add("There is some missing information! Fill all the fields before submitting please!", { variant: "error" });
        return false;
    }
    return true;
}
...
export const addEmployee = async (notification, navigator, employeeInfo)=>{
    if(validateEmployee(notification, employeeInfo)){
        ...
        if (response.status === 201) {
            notification.add("The employee was added successfully!", { variant: "success" });
...
export const getEmployeeInfo = async (notification, navigator, employeeId)=>{
    ...
    if (response.status === 200) {
        ...
        notification.add("Employee does not exist!", { variant: "error" });
        navigator("/app/employees/")
    }
    return data;
}

export const editEmployee = async (notification, navigator, employeeInfo, employeeId)=>{
    if(validateEmployee(notification, employeeInfo)){
        ...
        if (response.status === 200) {
            notification.add("The employee was updated successfully!", { variant: "success" });
            navigator("/app/employees/")
        } else if (response.status === 401) {
            logout(notification, navigator);
        } else {
...
export const deleteEmployee = async (notification, navigator, employeeId, dependency)=>{
    ...
    if (response.status === 200) {
        dependency(true);
        notification.add("The employee was deleted successfully!", { variant: "success" });
    } else if (response.status === 401) {
        logout(notification, navigator);
    } else {
        notification.add("The server could not handle the request!", { variant: "error" });
